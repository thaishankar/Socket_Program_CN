...
int phase1()
{
	...
	int numbytes;
	int yes=1;
	char send_to_dir[30]="File_Server2 42676\n";
...
//	hints.ai_flags = AI_PASSIVE;
	...
	if ((rv = getaddrinfo("localhost", FILE_SERVER2, &hints, &client)) != 0) 
	{
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
		return -1;
...
/*	void *addr;
	...
	printf("\nPhase 1: File Server2 has UDP Port Number %d and IP address %s \n", port,f2_ip);
*/
...
//Print IP
	...
	printf("\nPhase 1: File Server 2 has UDP Port Number %d and IP address %s \n", port,inet_ntoa(ip_addr.sin_addr));
...
//	hints.ai_flags=AI_PASSIVE;
	...
	if(status=getaddrinfo("localhost",TCP_FILE_SERVER2,&hints,&serv)!=0)
	{
...
/*	
	...
	inet_ntop(serv->ai_family,addr,f2_ip, sizeof f2_ip);
		...
		printf("\nPhase 3: File Server2 has TCP Port Number %d and IP address %s \n", port,f2_ip);
...
struct hostent *he;
	...
	printf("\nPhase 3: File Server 2 has TCP Port Number %d and IP address %s \n", port,inet_ntoa(ip_addr.sin_addr));	
...
//printf("Phase 3: End of Phase 3 for File Server 2\n");
// close while
