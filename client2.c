...
int phase2()
{
	...
	int numbytes;
	int yes=1;
	char send_to_dir[30]="Client2 doc2\n";
...
//	hints.ai_flags = AI_PASSIVE;
	...
	if ((rv = getaddrinfo("127.0.0.1", CLIENT2, &hints, &client)) != 0) 
	{
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
		return -1;
...
/*
	...
	printf("\nPhase 2: Client2 has UDP Port Number %d and IP address %s \n", port,f2_ip);
*/
struct hostent *he;
...
//	printf("talker: sent %d bytes to %s\n", numbytes, argv[1]);
	printf("Phase 2: The File Request from Client2 has been sent to the Directory Server\n");
	...
	printf("Phase 2: End of Phase 2 for Client2\n");		
	close(sockfd);
	return 1;
}
...
int phase3()
{
	...
	if(status=getaddrinfo(NULL,phase3_port,&hints,&client)!=0)
	{
...
/*
	...
	printf("\n\n\nPhase 3: Client2 has dynamic TCP Port Number %d and IP address %s \n", port,f2_ip);
*/
...
//Send Doc1 to File Server
	if (send(sockfd, "Client2 doc2", 30, 0) == -1)
	{
		perror("send");
		exit(1);
	}
	else
	printf("Phase 3: The File request from Client2 has been sent to File Server %s\n",phase3_server);
...
//	printf("client: received %s\n",buf);
	printf("Phase 3: End of Phase3 for Client 2\n");
	close(sockfd);
