...
int phase1()
{
	...
	int numbytes;
	int yes=1;
	char send_to_dir[30]="File_Server3 43676\n";
...
//	hints.ai_flags = AI_PASSIVE;
	...
	if ((rv = getaddrinfo("127.0.0.1", FILE_SERVER3, &hints, &client)) != 0) 
	{
		fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
		return -1;
...
/*
	...
	printf("\nPhase 1: File Server3 has UDP Port Number %d and IP address %s \n", port,f2_ip);
...
struct hostent *he;
	...
	printf("\nPhase 1: File Server 3 has UDP Port Number %d and IP address %s \n", port,inet_ntoa(ip_addr.sin_addr));	
...
int phase3()
{
	...
	if(status=getaddrinfo("127.0.0.1",TCP_FILE_SERVER3,&hints,&serv)!=0)
	{
...
/*	
	...
	printf("\nPhase 3: File Server3 has TCP Port Number %d and IP address %s \n", port,f2_ip);
	...
	*/
	...
	printf("\nPhase 3: File Server 3 has TCP Port Number %d and IP address %s \n", port,inet_ntoa(ip_addr.sin_addr));
...
//printf("Phase 3: End of Phase 3 for File Server 3\n");
// close while
